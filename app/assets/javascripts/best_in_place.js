...
BestInPlaceEditor.forms = {
  ...
  "textarea" : {
    ...
    blurHandler : function(event) {
      if (event.data.editor.okButton) {
        event.data.editor.blurTimer = setTimeout(function () {
          ...
          if (!event.data.editor.userClicked) {
            //event.data.editor.abortIfConfirm(); //mpl
            processCancelEvent(event);
          }
        }, 500);
      } else {
        if (event.data.editor.cancelButton) {
    ...
    cancelButtonHandler : function(event) {
      ...
      //mpl
      processCancelEvent(event);
/*      newValue = $(this).parents('span').find('textarea')[0].value;
      oldValue = event.data.editor.oldValue;
      //console.log( newValue + " " + oldValue + " " + (newValue==oldValue));
      if(newValue == oldValue){
        event.data.editor.abort(); //IfConfirm(); //mpl
      }else{
        event.data.editor.abortIfConfirm(); //mpl
      }
 */
  ...
  var processCancelEvent = function(event){
        if(processing){
          // Stops a double call in the case of esc key
          return;
        }else{
          processing = true;
        }
        //Activator is the span
        activator = event.data.editor.activator;
        newValue = activator.find('textarea')[0].value;
        //newValue = $(this).parents('span').find('textarea')[0].value;
        oldValue = event.data.editor.oldValue;
        //use underscore.js to remove html entities like &lt;
        if(newValue == _.unescape(oldValue)){
          event.data.editor.abort(); //IfConfirm(); //mpl
        }else{
          event.data.editor.abortIfConfirm(); //mpl
        }
